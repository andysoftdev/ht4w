...
static PATCH rls_patches[] = 
{
  ...
  {"exit",            (FARPROC) hoard_exit,      0, 0},
  {"_exit",           (FARPROC) hoard__exit,     0, 0},
  {"_onexit",         (FARPROC) hoard_onexit,    0, 0},
  {"_expand",         (FARPROC) hoard_expand,	   0, 0},

  // operator new, new[], delete, delete[].

#ifdef _WIN64

  {"??2@YAPEAX_K@Z",  (FARPROC) hoard_malloc,    0, 0},
  {"??_U@YAPEAX_K@Z", (FARPROC) hoard_malloc,    0, 0},
  {"??3@YAXPEAX@Z",   (FARPROC) hoard_free,      0, 0},
  {"??_V@YAXPEAX@Z",  (FARPROC) hoard_free,      0, 0},

#else

  {"??2@YAPAXI@Z",    (FARPROC) hoard_malloc,    0, 0},
  {"??_U@YAPAXI@Z",   (FARPROC) hoard_malloc,    0, 0},
  {"??3@YAXPAX@Z",    (FARPROC) hoard_free,      0, 0},
  {"??_V@YAXPAX@Z",   (FARPROC) hoard_free,      0, 0},
  ...
  //{"??2@YAPAXIABUnothrow_t@std@@@Z",  (FARPROC) hoard_malloc, 0, 0},
  //{"??_U@YAPAXIABUnothrow_t@std@@@Z", (FARPROC) hoard_malloc, 0, 0},

  // the nothrow variants delete, delete[].

  //{"??3@YAXPAXABUnothrow_t@std@@@Z",  (FARPROC) hoard_malloc, 0, 0},
  //{"??_V@YAXPAXABUnothrow_t@std@@@Z", (FARPROC) hoard_malloc, 0, 0},

  {"_msize",	(FARPROC) hoard_malloc_usable_size, 0, 0},
  {"calloc",	(FARPROC) hoard_calloc,     0, 0},
  {"malloc",	(FARPROC) hoard_malloc,     0, 0},
  {"realloc",	(FARPROC) hoard_realloc,    0, 0},
  {"free",	(FARPROC) hoard_free,         0, 0},
  {"_recalloc", (FARPROC) hoard_recalloc, 0, 0},
  {"_putenv",	(FARPROC) hoard__putenv,    0, 0},
  {"getenv",	(FARPROC) hoard_getenv,     0, 0},
};
    ...
    if (RlsCRTLibrary) {
      for (int i = 0; i < sizeof(rls_patches) / sizeof(*rls_patches); i++) {
        if (rls_patches[i].original = GetProcAddress(RlsCRTLibrary, rls_patches[i].import)) {
          rls_patches[i].module = RlsCRTLibrary;
...
extern "C" {
  ...
  void hoard_exit (int code) {
    exit_t fp;
    {
      ...
      UnPatchIt(&rls_patches[0]);
      fp = (exit_t)rls_patches[0].original;
    }
    (*fp)(code);
  }
  ...
  void hoard__exit (int code) {
    ...
    {
      HL::Guard<TheLockType> g (exitLock);
      UnPatchIt(&rls_patches[1]);
      fp = (exit_t)rls_patches[1].original;
    }
    (*fp)(code);
  }
