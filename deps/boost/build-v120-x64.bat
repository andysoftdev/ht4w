...
:vcvarsall
set VCVARSALL="c:\Program Files\Microsoft Visual Studio 12.0\VC\vcvarsall.bat"
...
if not exist "%THIS_PATH%\stage\%PLATFORMTOOLSET%" md "%THIS_PATH%\stage\%PLATFORMTOOLSET%"
if not exist "%THIS_PATH%\stage\%PLATFORMTOOLSET%\%PLATFORM%" md "%THIS_PATH%\stage\%PLATFORMTOOLSET%\%PLATFORM%"

bjam --toolset=msvc-12.0 --build-dir="%THIS_PATH%\..\..\build\%PLATFORMTOOLSET%\deps\boost\%PLATFORM%" --stagedir="%THIS_PATH%\stage\%PLATFORMTOOLSET%\%PLATFORM%" --without-python --without-mpi -sNO_COMPRESSION=0 -sNO_ZLIB=0 -sZLIB_SOURCE="%ZLIB_SOURCE%" -sNO_BZIP2=0 -sBZIP2_SOURCE="%BZIP2_SOURCE%" address-model=64 debug link=static runtime-link=shared cflags=/GF cxxflags=/GF stage
...
:missing_vcvarsall
echo missing vcvarsall.bat, install VC2013 or adjust the VCVARSALL variable
goto done
