...
bjam --toolset=msvc-14.2 --build-dir="%THIS_PATH%\..\..\build\%PLATFORMTOOLSET%\deps\boost\%PLATFORM%" --stagedir="%THIS_PATH%\stage\%PLATFORMTOOLSET%\%PLATFORM%" --without-python --without-mpi -sNO_COMPRESSION=0 -sNO_ZLIB=0 -sZLIB_SOURCE="%ZLIB_SOURCE%" -sNO_BZIP2=0 -sBZIP2_SOURCE="%BZIP2_SOURCE%" address-model=64 debug link=static runtime-link=shared cflags=/GF cxxflags=/GF stage
