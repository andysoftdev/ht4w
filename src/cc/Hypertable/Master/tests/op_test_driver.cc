...
#else
    ::DeleteFile(format("%s.tmp", basename.c_str()).c_str());
    cmd = format("ren %s.output %s.tmp", basename.c_str(), basename.c_str());
#endif
...
#else
    cmd = format("sed.exe -e \"s/[0-9/]* move range/<tid> move range/g\" %s.tmp > %s.output",
                 basename.c_str(), basename.c_str());
#endif
    if ((ret = system(cmd.c_str())) != 0) {
      cout << "Shell command \"" << cmd << "\" returned " << ret << endl;
...
#else
    cmd = format("del %s.tmp", basename.c_str());
#endif
    if ((ret = system(cmd.c_str())) != 0) {
      cout << "Shell command \"" << cmd << "\" returned " << ret << endl;
      return false;
...
#else
    cmd = format("fc %s.output %s.golden", basename.c_str(), basename.c_str());
#endif
    if ((ret = system(cmd.c_str())) != 0) {
      cout << "Shell command \"" << cmd << "\" returned " << ret << endl;
      return false;
...
void balance_plan_authority_test(ContextPtr &context) {
  ...
  String cmd = "fc balance_plan_authority_test.sed.output balance_plan_authority_test.golden";
