...
namespace Hypertable {
  ...
  inline uint64_t get_file_length(const WIN32_FIND_DATA& ffd) {
     ULARGE_INTEGER ull;
  ...
  inline time_t get_last_access_time(const WIN32_FIND_DATA& ffd) {
    return filetime_to_time_t(ffd.ftLastAccessTime);
  }

  inline bool is_directory(const WIN32_FIND_DATA& ffd) {
